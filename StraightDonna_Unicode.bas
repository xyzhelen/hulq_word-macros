...
Sub Straight_Char()
	...
	Selection.Find.Execute Replace:=wdReplaceAll
	With Selection.Find
		'greek capital Delta (not to be confused with increment)
		.Text = ChrW(916)
	...
	Selection.Find.Execute Replace:=wdReplaceAll
	With Selection.Find
		'fraction slash to shwa-acute (not to be confused with slash) (in font spec but doesnt seem to work?)
		.Text = ChrW(8260)
...
Sub Imposter_Char()
	...
	End With
	Selection.Find.Execute Replace:=wdReplaceAll
	'clear formatting dialog for the user	Selection.Find.ClearFormatting
	Selection.Find.Replacement.ClearFormatting
